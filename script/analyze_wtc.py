...
files = [os.path.join(src_base, f) for f in representative_files]

result = analyze_corpus(files)

if result:
    print("=== ANALISIS DE PATRONES RITMICOS - WTC Libro I ===\n")
    ...
    for (n1, n2, n3), count in result['trigrams'].most_common(10):
        print(f"   {n1} -> {n2} -> {n3}: {count}")
    
    print(f"\n4. RUNS DE MISMA FIGURA:")
    print(f"   Longitud promedio: {result['avg_run_length']:.2f}")
    print(f"   Distribucion de longitudes:")
    for length in sorted(result['distribution_runs'].keys()):
        count = result['distribution_runs'][length]
        print(f"     Longitud {length}: {count} runs")
    
    print(f"\n5. PATRONES PUNTEADOS:")
    print(f"   Ocurrencias: {result['dotted_count']}")
    ...
    if result['dotted_patterns']:
        ...
        for (n1, n2), count in dotted_bigrams.most_common(5):
            print(f"     {n1} (.) -> {n2}: {count}")
else:
