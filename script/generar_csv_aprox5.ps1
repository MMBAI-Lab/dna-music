# Genera notas_256_GROOVE_aprox5.csv
# Pitch quantizado a Re menor natural, registros soprano/bajo de aprox5

$base = 'e:/Documentos/2026/PROYECTO - ABC musica Molla'

# Re menor natural: C=0, D=2, E=4, F=5, G=7, A=9, Bb=10
$dMinor = @(0, 2, 4, 5, 7, 9, 10)
$noteNames = @('C','C#','D','D#','E','F','F#','G','G#','A','A#','B')

function Snap-ToDMinor($midi) {
    $chroma = $midi % 12
    $base_m = $midi - $chroma
    $bestC = $dMinor[0]; $bestD = 99
    foreach ($c in $dMinor) {
        $d = [math]::Abs($chroma - $c)
        if ($d -gt 6) { $d = 12 - $d }
        if ($d -lt $bestD) { $bestD = $d; $bestC = $c }
    }
    $n = $base_m + $bestC
    if (($chroma - $bestC) -gt 6)  { $n += 12 }
    if (($bestC - $chroma) -gt 6)  { $n -= 12 }
    return $n
}

function Force-Register($midi, $center, $lo, $hi) {
    $chroma = $midi % 12
    $best = $midi; $bestD = 999
    for ($oct = 0; $oct -le 9; $oct++) {
        $c = $oct * 12 + $chroma
        if ($c -lt $lo -or $c -gt $hi) { continue }
        $d = [math]::Abs($c - $center)
        if ($d -lt $bestD) { $bestD = $d; $best = $c }
    }
    return $best
}

function Midi-ToName($midi) {
    $chroma = $midi % 12
    $octave = [int]($midi / 12) - 1
    return $noteNames[$chroma] + $octave
}

foreach ($groove in @('majorgroove','minorgroove')) {
    # Registros: soprano D4-D6 (62-86, centro 69) / bajo D2-D4 (38-62, centro 50)
    if ($groove -eq 'majorgroove') { $lo=62; $hi=86; $center=69; $voice='soprano' }
    else                           { $lo=38; $hi=62; $center=50; $voice='bajo'    }

    # Leer CSV aprox1_2 como base
    $lines = Get-Content "$base/data/notas_256_${groove}_aprox1_2.csv"
    $result = @('tetranucleotide,canonico,nota_original,nota_aprox5,midi_aprox5,voz')

    foreach ($line in $lines[1..($lines.Length-1)]) {
        $f = $line -split ','
        $tetra   = $f[0]
        $canon   = $f[1]
        $nota_orig = $f[4]
        # Parse MIDI from original note name
        if ($nota_orig -match '^([A-G]#?)(\d)$') {
            $noteMap = @{C=0;'C#'=1;D=2;'D#'=3;E=4;F=5;'F#'=6;G=7;'G#'=8;A=9;'A#'=10;B=11}
            $midi_orig = 12 * ([int]$Matches[2] + 1) + $noteMap[$Matches[1]]
            $midi_snapped = Snap-ToDMinor $midi_orig
            $midi_final   = Force-Register $midi_snapped $center $lo $hi
            $nota_final   = Midi-ToName $midi_final
            $result += "$tetra,$canon,$nota_orig,$nota_final,$midi_final,$voice"
        }
    }

    $outFile = "$base/data/notas_256_${groove}_aprox5.csv"
    $result -join "`n" | Set-Content -NoNewline $outFile -Encoding UTF8
    Write-Host "Guardado: $outFile ($($result.Count - 1) filas)"
}
